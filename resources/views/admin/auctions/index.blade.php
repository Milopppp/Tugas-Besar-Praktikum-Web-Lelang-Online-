            ...
            <tbody class="divide-y divide-gray-100">
                ...
                <tr class="hover:bg-gray-50 transition">
                    <td class="px-6 py-4">
                        ...
                        <div class="text-[10px] text-gray-400">Harga Awal: Rp {{ number_format($auction->item->harga_awal) }}</div>
                    </td>
                    <td class="px-6 py-4 text-sm text-gray-600">
                        {{ \Carbon\Carbon::parse($auction->tgl_akhir)->format('d M Y, H:i') }}
                    </td>
                    <td class="px-6 py-4 text-right font-bold text-blue-600">
                        Rp {{ number_format($auction->harga_akhir ?? $auction->item->harga_awal) }}
                    </td>
                    ...
                    <td class="px-6 py-4 text-center">
                        <div class="flex justify-center gap-2">
                            <a href="{{ route('admin.auctions.show', $auction->id) }}" class="p-2 text-blue-600 hover:bg-blue-50 rounded-lg transition" title="Lihat Penawar">
                                <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z"/><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z"/></svg>
                            </a>

                            <a href="{{ route('admin.auctions.edit', $auction->id) }}" class="p-2 text-amber-600 hover:bg-amber-50 rounded-lg transition" title="Ubah Batas Waktu">
                            ...
                            <form action="{{ route('admin.auctions.destroy', $auction->id) }}" method="POST" class="inline">
                                @csrf @method('DELETE')
                                <button type="submit" class="p-2 text-red-600 hover:bg-red-50 rounded-lg transition" onclick="return confirm('Hapus sesi ini?')" title="Hapus">
                ...
                @empty
                <tr>
                    <td colspan="5" class="px-6 py-10 text-center text-gray-400">Belum ada sesi lelang yang dibuat.</td>
                </tr>
                @endforelse
            </tbody>
        </table>
